# macOS computer use — reference material (not the build target)

Status: **Reference only.** Synara's computer use targets Linux; the authoritative design is `docs/computer-use-design.md`. This file preserves the macOS/Codex reverse-engineering research it draws on, and would be the starting point if a macOS backend is ever added.
