...
- **Phase A — windowed nested session.** Smallest shippable slice, no new native code. `nestedKWinSession.ts` already boots a private compositor; keep it and drop `--virtual`, and `kwin_wayland` runs as an ordinary Wayland client of mutter or sway — a real window on the human's desktop containing an isolated agent desktop, with the dedicated seat, ghost cursor, in-plugin capture, AT-SPI, clipboard, occlusion, idle timeout, and release hotkey all intact. This delivers the design doc's Tier 2 promise (agent-launched apps, human's session provably untouched) on GNOME and wlroots today, differing only in that the apps live inside one host window rather than mixed among the human's. Cost: `kwin_wayland` + `kwin-devel` + the plugin build on a non-KDE host. Work: a windowed mode flag, selection, availability copy naming the missing package, panel copy.
- **Phase B1 — skeleton.** `PortalComputerBackend` with the probe, capabilities, `awaiting-consent`, and precise unavailability messages; the shared extractions out of `KWinComputerBackend` and `nestedKWinSession`; no behavior change on KDE, regression-tested.
- **Phase B2 — wlroots providers plus the headless-sway lane.** No portal, no consent, all four capabilities, and fully CI-testable. Deliberately before GNOME: it proves the provider architecture against a real compositor in CI.
- **Phase B3 — GNOME portal providers** behind the fake-portal service, then the live spike.
