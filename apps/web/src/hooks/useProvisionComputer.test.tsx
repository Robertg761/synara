...
describe("useProvisionComputer", () => {
  ...
  it("shows one provision as pending on every surface that offers it", () => {
    reset();
    // Never settles: the question is what the other surface shows while the
    // installer is still running.
    provisionComputer.mockReturnValue(new Promise(() => {}));
    const queryClient = createQueryClient();

    mountProvisionHook(queryClient).provision();

    // The transcript card and the settings panel are routinely on screen
    // together. Owning the mutation per instance left the one that did not
    // press Set up with an enabled button, and pressing it re-entered the
    // installer behind the permission dialog the user was already looking at.
    const other = mountProvisionHook(queryClient);
    expect(other.isPending).toBe(true);
    other.provision();
