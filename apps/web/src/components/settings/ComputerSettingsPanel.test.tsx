...
function binding(): AppSettingsBinding {
  return {
    settings: { autoOpenComputerPane: true },
    defaults: { autoOpenComputerPane: true },
...
describe("ComputerSettingsPanel", () => {
  ...
  it("says computer control is not a switch", () => {
    // The property the product owner decided; the panel is where a user goes
    // looking for the toggle that no longer exists.
    expect(render({ status: status() })).toContain("Computer control is not a switch");
  });
});
