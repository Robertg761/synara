...
function status(overrides: Partial<ComputerStatusResult> = {}): ComputerStatusResult {
  return {
    ...
    // The Mac backend has an installer, which is what makes Set up a real
    // button rather than a guaranteed error.
    provisionable: true,
    ...overrides,
  };
}

/** The Set up button itself, as opposed to the rows that tell you to press it. */
function offersSetUpButton(markup: string): boolean {
  return markup.includes(">Set up<");
}
...
describe("ComputerSettingsPanel", () => {
  ...
  it("asks to check access before a shipped helper has actually connected", () => {
    ...
    expect(offersSetUpButton(markup)).toBe(true);
  });
  ...
  it("explains rather than offering a Set up that would only ever fail", () => {
    // The default Linux host: no Wayland session Synara can drive, so the
    // backend resolves to nothing and there is no installer behind the button.
    // The panel used to draw it anyway and every press answered "This desktop
    // backend has nothing to install."
    const markup = render({
      status: status({
        availability: { kind: "backend-unavailable", message: "No Wayland session." },
        capabilities: capabilities({ capture: false, input: false }),
        provisionable: false,
      }),
    });
    expect(offersSetUpButton(markup)).toBe(false);
    expect(markup).toContain("nothing here for Synara to install");
  });

  it("says nothing about installing on a desktop that is simply ready", () => {
    expect(render({ status: status() })).not.toContain("nothing here for Synara to install");
  });
