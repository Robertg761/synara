...
export function ComputerSettingsPanel({
  ...
  /**
   * The desktop is short of something and Synara has no installer for it — a
   * Linux host with no Wayland compositor Synara can drive is the ordinary
   * case. The panel used to draw Set up here anyway and the server answered
   * every press with "This desktop backend has nothing to install", which reads
   * as a broken button rather than as the true answer. The availability row
   * above already says what is wrong; this says who has to fix it.
   */
  const setupNotOffered =
    !statusQuery.isError && computerDesktopIsUnready(status) && status?.provisionable !== true;
    ...
    <div className="space-y-6">
      ...
      >
        <SettingsCard>
          ...
          {setupNotOffered ? (
            <SettingsRow
              title="There is nothing here for Synara to install"
              description="Synara sets up its own compositor plugin and helpers, but it cannot create the desktop session they attach to. Start a supported Wayland session — KWin or Hyprland — or use Synara's own nested desktop, then press Refresh."
            />
          ) : null}
          {backend ? (
            <SettingsRow
