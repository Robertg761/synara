...
import { createComputerScrollBatch } from "./computer/computerScrollBatch";
...
export default function ComputerPanel(props: {
  ...
  // Held past the moment it clears so the notice has something to animate out.
  const inputPause = threadState?.inputPause;
  const lastInputPauseRef = useRef(inputPause);
  if (inputPause) lastInputPauseRef.current = inputPause;
  const shownInputPause = inputPause ?? lastInputPauseRef.current;
  ...
  // Memoised on the two numbers, never on the object: `threadState.screenSize`
  // is a fresh object on every state push, and anything keyed on its identity
  // is recomputed several times a second on an idle desktop — which used to
  // re-subscribe the wheel listener mid-gesture and lose the batched deltas.
  ...
  const containRect = useMemo(
    () =>
      screenWidth !== undefined && screenHeight !== undefined
        ? computerContainRect({
            source: { width: screenWidth, height: screenHeight },
            containerWidth: viewportSize.width,
            containerHeight: viewportSize.height,
          })
        : null,
    [screenHeight, screenWidth, viewportSize.height, viewportSize.width],
  );
  ...
  // The blocked states the user can clear without leaving the pane. Same
  // server-side provision, same single-flight, same words as the chat card's
  // Set up and the settings panel's.
  //
  // Which of them Set up can actually clear is a property of the backend
  // (`provisionable`) that thread state does not carry, so the shared status
  // query supplies it — the same query and cache entry the settings panel and
  // the chat card use, asked only while this pane is showing something other
  // than a live desktop. Gating on `permission-required` alone was wrong in
  // both directions: a Mac missing only Screen Recording answers `available`
  // with `captureAvailable: false`, and the blocked view told the user to
  // choose Set up beside no button at all.
  const backendStatus = useQuery({
    ...computerStatusQueryOptions(),
    enabled: availabilityView.kind !== "ready",
  }).data;
  const permissionRequired =
    threadState?.availability.kind === "permission-required" ? threadState.availability : null;
  const needsSetup = computerStatusNeedsSetup(
    threadState
      ? { ...threadState, ...(backendStatus ? { provisionable: backendStatus.provisionable } : {}) }
      : backendStatus,
  );
  ...
  // Owned by the pane, not by the wheel effect: the effect re-subscribes
  // whenever the desktop pushes new geometry, and a burst still accumulating
  // has to outlive that.
  const scrollBatch = useMemo(() => createComputerScrollBatch(), []);

  const sendInput = useCallback(
    (send: () => Promise<ComputerActionResult>) => {
      // Both delays exist to coalesce, not to reorder: a click or a keystroke
      // that follows a scroll must reach the seat after it.
      clickSequence.flush();
      scrollBatch.flush();
    ...
    [inputQueue, clickSequence, scrollBatch],
  );

  const region = useMemo(
    () =>
      screenWidth !== undefined && screenHeight !== undefined
        ? computerStreamRegion({ width: screenWidth, height: screenHeight })
        : null,
  ...
  }, [interactive, inputQueue, clickSequence, scrollBatch]);
  useEffect(
    () => () => {
      clickSequence.clear();
      scrollBatch.clear();
    ...
    [inputQueue, clickSequence, scrollBatch, threadId],
  );
  ...
  const handleKeyDown = useCallback(
    (event: React.KeyboardEvent<HTMLCanvasElement>) => {
      if (!interactive) return;
      // Read out field by field rather than passed through: React's synthetic
      // keyboard event has no `isComposing`, and the one on the native event is
      // what tells an IME's keystrokes apart from the user's.
      const command = computerKeyCommand({
        key: event.key,
      ...
      // A key the seat must not be asked to express is left to the browser
  ...
  useEffect(() => {
    ...
    const onWheel = (event: WheelEvent) => {
      ...
      if (!point) return;
      event.preventDefault();
      scrollBatch.add(point, computerWheelScrollDelta(event), (scroll) =>
        sendInput(() => ensureNativeApi().computer.inputScroll(scroll)),
      );
    };
    ...
    return () => {
      ...
      // Send what the user already scrolled rather than dropping it with the
      // listener: this effect is torn down and rebuilt by ordinary desktop
      // activity, not only when the pane closes.
      scrollBatch.flush();
    };
  }, [desktopPointFromEvent, interactive, scrollBatch, sendInput]);
  ...
  return (
    <DiffPanelShell mode={props.mode} header={header}>
      {/* Opens and closes with the app's one disclosure motion rather than
          appearing instantly: it pushes the desktop image down, and a viewport
          that jumps under the user's cursor mid-gesture is worse than one that
          moves. The pause is held for the closing frames so there is something
          to animate out. */}
      <DisclosureRegion open={inputPause !== undefined} className="shrink-0">
        {shownInputPause ? (
          <ComputerInputPauseNotice
            key={shownInputPause.windowId ?? "unknown"}
            pause={shownInputPause}
