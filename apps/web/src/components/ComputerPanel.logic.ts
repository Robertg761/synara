...
/**
 * Typed on the four fields rather than on `ComputerStatusResult`, because the
 * thread-scoped state a chat receives by push carries three of them and has to
 * be answerable by the same questions — the chat's setup card reads the live
 * thread state, the settings panel reads the polled status, and a second copy
 * of these rules for the other shape is how they would start disagreeing.
 */
...
/**
 * Whether this desktop is short of something an agent needs — the backend, a
 * grant, working capture, the clipboard tool.
 *
 ...
 * capabilities-only test never notices a blocked desktop at all. What separates
 * "nothing to do" from "not ready" is whether a backend resolved, whether it
 * can currently capture, and only then whether it claims the two abilities. A
 * platform that can never run this is not a machine with something missing.
 */
export function computerDesktopIsUnready(status: ComputerSetupProbe | undefined): boolean {
  ...
  // A compositor-plugin desktop with no clipboard is a half-installed one: the
  // plugin loaded but the companion tool it shells out to did not.
...
/**
 * Whether pressing "Set up" could change any of that.
 *
 * `provisionable` is the server telling us its backend has a `provision` at all.
 * Without this gate the default Linux host — no Wayland compositor Synara can
 * reach, so `backend-unavailable` and no installer behind it — drew a Set up
 * button on both the settings panel and the chat card, and every press came
 * back "This desktop backend has nothing to install." A button whose only
 * outcome is an error is worse than an explanation.
 *
 * Thread state (`ThreadComputerState`) does not carry `provisionable`, so a
 * surface holding only that has to pair it with the shared status query before
 * asking this question; `computerDesktopIsUnready` is the part that answers
 * from thread state alone.
 */
export function computerStatusNeedsSetup(status: ComputerSetupProbe | undefined): boolean {
  return status?.provisionable === true && computerDesktopIsUnready(status);
}
...
export function computerControlReadiness(
  ...
  // Readiness, not actionability: a desktop nobody can install is still not
  // ready, and saying "ready" there would be the card lying about the machine.
  return computerDesktopIsUnready(state) ? "needs-setup" : "ready";
}
...
export interface ComputerKeyEventLike {
  ...
  /** Auto-repeat from a key the user is holding down. */
  readonly repeat?: boolean;
  /** The keydown belongs to an in-progress IME composition. */
  readonly isComposing?: boolean;
  /** 229 while composing, on engines that leave `isComposing` unset. */
...
/** The legacy "the key belongs to an IME composition" sentinel. */
const IME_COMPOSITION_KEY_CODE = 229;

/** Code points, not UTF-16 units, so an emoji counts as the one key it is. */
function isSingleCharacter(key: string): boolean {
  const iterator = key[Symbol.iterator]();
  return iterator.next().done === false && iterator.next().done === true;
}

/**
 * Translates a keydown into one backend key press, or null for keys the seat
 * must not be asked to express. A null must not be swallowed by the pane:
 * leaving it to the browser is the honest outcome.
 *
 * Three kinds of null.
 *
 * Auto-repeat, because a held key produces keydowns faster than the seat can
 * inject them and the pane's queue is 24 deep — holding Backspace for a second
 * overflowed it and turned a stuck key into a "the desktop is busy" error. The
 * backend has its own repeat once the key is down.
 *
 * IME composition, because the composing keystrokes are input to the IME, not
 * to the desktop: forwarding them types the user's romaji into the remote
 * window alongside whatever the IME finally commits.
 *
 * And keys with no character and no name — bare modifiers, dead keys — which
 * the seat has no code for and the browser needs to see anyway.
 *
 * A character outside ASCII is *not* one of them. The pane used to drop them,
 * so a German or French keyboard could not type its own letters into the
 * desktop and got nothing back to explain why. Whether the seat can synthesize
 * a character is the backend's question — it knows the keymap, and it answers
 * with a message the pane already shows.
 */
export function computerKeyCommand(event: ComputerKeyEventLike): ComputerKeyCommand | null {
  if (event.repeat === true) return null;
  if (event.isComposing === true || event.keyCode === IME_COMPOSITION_KEY_CODE) return null;

  const key = resolveComputerKeyName(event.key);
  if (key === null) return null;

  const character = isSingleCharacter(key);
  ...
  if (event.shiftKey && !character) modifiers.push("shift");
...
function resolveComputerKeyName(key: string): string | null {
  ...
  if (isSingleCharacter(key)) return key;
