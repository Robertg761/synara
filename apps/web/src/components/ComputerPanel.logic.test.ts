...
/**
 * Thread state plus the one field only `computer.getStatus` carries: whether
 * this backend has an installer behind the Set up button at all.
 */
function probe(overrides: Partial<ThreadComputerState> = {}, provisionable = true) {
  return { ...state(overrides), provisionable };
}
...
describe("computer pane wheel and key mapping", () => {
  ...
  it("forwards a character the US-QWERTY table has no code for", () => {
    // Dropping these silently meant a German or French keyboard could not type
    // its own letters into the desktop and got nothing back to say why.
    // Whether the seat can synthesize one is the backend's question — it knows
    // the keymap, and it answers with a message the pane already shows.
    expect(keyEvent("é")).toEqual({ key: "é", modifiers: [] });
    expect(keyEvent("ß")).toEqual({ key: "ß", modifiers: [] });
    expect(keyEvent("Ä", { shiftKey: true })).toEqual({ key: "Ä", modifiers: [] });
    // One code point, two UTF-16 units: still one keystroke.
    expect(keyEvent("😀")).toEqual({ key: "😀", modifiers: [] });
  });

  it("ignores auto-repeat from a held key", () => {
    // A held key produces keydowns far faster than the seat can inject them.
    // The pane's queue is 24 deep, so a second on Backspace overflowed it and
    // turned a stuck key into "the desktop is busy".
    expect(keyEvent("Backspace", { repeat: true })).toBeNull();
    expect(keyEvent("a", { repeat: true })).toBeNull();
    expect(keyEvent("Backspace")).toEqual({ key: "backspace", modifiers: [] });
  });

  it("ignores keystrokes that belong to an IME composition", () => {
    // Those strokes are input to the IME, not to the desktop: forwarding them
    // types the romaji into the remote window beside whatever the IME commits.
    expect(keyEvent("k", { isComposing: true })).toBeNull();
    expect(keyEvent("Process", { keyCode: 229 })).toBeNull();
    expect(keyEvent("Enter", { isComposing: true })).toBeNull();
  });
});
...
describe("computerStatusNeedsSetup", () => {
  ...
  it("says no on a host that could never have a desktop backend", () => {
    ...
    expect(
      computerStatusNeedsSetup(
        probe({ availability: { kind: "unsupported-platform", platform: "win32" } }),
      ),
    ).toBe(false);
  });

  it("says no on a ready desktop", () => {
    expect(computerStatusNeedsSetup(probe())).toBe(false);
  });
  ...
  it("says yes on a withheld grant and on a backend that is not there", () => {
    ...
    expect(
      computerStatusNeedsSetup(
        probe({ availability: { kind: "backend-unavailable", message: "no helper" } }),
      ),
    ).toBe(true);
  });

  it("says no on a broken desktop no installer can reach", () => {
    // The default Linux host: no Wayland compositor Synara can drive, so the
    // backend resolves to nothing and there is no `provision` behind the
    // button. Offering Set up there produced one error per press.
    const broken = { availability: { kind: "backend-unavailable" as const, message: "no helper" } };
    expect(computerStatusNeedsSetup(probe(broken, false))).toBe(false);
    expect(computerStatusNeedsSetup(state(broken))).toBe(false);
    // The desktop is still broken, which is a separate question and the one the
    // chat card's "is it ready yet" reads.
    expect(computerDesktopIsUnready(state(broken))).toBe(true);
  });
  ...
  it("says yes when the desktop is driveable but blind", () => {
    ...
    expect(
      computerStatusNeedsSetup(
        probe({ health: { ...connectedHealth(), captureAvailable: false } }),
      ),
    ).toBe(true);
  });
...
describe("Linux clipboard setup", () => {
  ...
  it("does not require clipboard on a backend that does not offer it", () => {
    const current = probe({ capabilities: { ...state().capabilities, clipboard: false } });
