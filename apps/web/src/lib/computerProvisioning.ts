...
import { computerDesktopIsUnready } from "~/components/ComputerPanel.logic";
...
export function computerProvisionOutcome(
  ...
  // Readiness, not actionability: the provision already ran, so whether the
  // backend has an installer is settled. What is left to report is whether the
  // desktop came back short of anything.
  return computerDesktopIsUnready(result.status) ? "incomplete" : "ready";
}
