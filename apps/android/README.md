...
Tabs, navigation, Back/Forward, reload, and screenshot attachments use the shared browser contracts. Android keeps one guest renderer alive at a time, with at most 32 tabs across 16 thread workspaces. At the workspace or tab limit, the least recently used hidden workspace is discarded; the displayed workspace is retained. State queries do not reserve a workspace. Inactive tabs save up to 256 KiB of native navigation history each; switching tabs reloads the page when necessary. Browser sessions reset with the app renderer and are not persisted across process termination. Native view bounds follow the shared panel; overlays and backgrounding hide the guest.
