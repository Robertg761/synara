...
public:
    ...
    // Whether the call being processed comes from the authenticated server
    // connection. For methods that answer everyone but say more to the server.
    bool permits() const {
        return !caller.empty() && caller == object.getCurrentlyProcessedMessage().getSender();
    }
    void require() const {
        if (!permits())
