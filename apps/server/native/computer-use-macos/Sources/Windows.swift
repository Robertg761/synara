...
enum Windows {
  ...
  private static func enumerate() -> [DesktopWindow] {
    // Deliberately NOT `.optionOnScreenOnly`. With that flag every returned
    // window is by definition on screen, so `onScreen` was structurally always
    // true and `minimized` always false — a minimized window simply vanished
    // from `list-windows`, and an agent asked to "bring the minimized Notes
    // window forward" could not see it existed. The list is re-ordered below so
    // the on-screen windows keep their front-to-back stacking.
