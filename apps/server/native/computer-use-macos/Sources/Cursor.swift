...
//     one integrating.
//
// Not done, deliberately: the reference's `orderWindow:relativeTo:` trick to sit
// above only the target. `NSWindow.order(_:relativeTo:)` is documented to accept
// only window numbers belonging to *this* application, so ordering above another
// app's window needs the CGS SPI. `orderFrontRegardless` never activates the
// helper and is enough; the overlay is 126 pt of mostly-transparent window.
...
final class AgentCursor {
  ...
  //   3. **Nothing runs at rest.** The link is invalidated when the tip settles,
  //      so an idle helper costs zero CPU.
  //
