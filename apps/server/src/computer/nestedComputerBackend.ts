...
export interface NestedComputerBackendOptions {
  ...
  /**
   * The KWin on disk, which is the one this backend is about to boot — and so
   * the one the setup gate and the prebuilt match are about here, where there
   * is no already-running compositor to be a different version from it.
   */
  readonly installedKwinVersion?: KWinComputerBackendOptions["installedKwinVersion"];
...
export class NestedComputerBackend extends KWinComputerBackend {
  ...
  constructor(options: NestedComputerBackendOptions = {}) {
    ...
    super({
      // One description of how a nested session is driven, shared with the
      // integration test that boots a real compositor: the session is resolved
      // on every use rather than captured, because this backend is built before
      // any session exists and outlives each one it boots.
      ...nestedKWinBackendOptions(() => ref.session, {
        mode,
      ...
      ...(options.installedKwinVersion
        ? { installedKwinVersion: options.installedKwinVersion }
        : {}),
  ...
  /**
   * The passive status read, which the settings screen polls on a timer.
   *
   ...
   * running, click Refresh" for the thirty seconds a cold start takes, and the
   * Refresh it invites does nothing but join the boot already running. Health
  ...
  /**
   * Disposal has to catch a boot that is still running, not only a session that
   * finished booting. `startSession` spawns a bus and a compositor well before
   * it resolves, so a server told to shut down mid-boot used to return from
   * `dispose()` with both processes still starting and nothing left holding a
   * reference to them — the exact orphan pair the session marker exists to
   * catch, produced by the orderly path rather than by a crash.
   */
  override async dispose(): Promise<void> {
    ...
    const session = this.ref.session ?? booted;
  ...
  private async hasVerifiedPrebuilt(): Promise<boolean> {
    ...
    // The bytes are checked, not merely selected: a shipped binary that fails
    // its checksum is a build this machine must not skip its toolchain for.
    return (await readVerifiedPrebuilt(join(root, build.file), build.sha256)) !== undefined;
  }
  ...
  private async ensureSession(automatic: boolean): Promise<NestedKWinSession> {
    ...
    if (automatic && this.sessionStarted) {
      ...
      // just closed, or crash-loop a headless compositor that just died, so
      // report dormancy — which stands the loop down — and leave the boot to
      // the next real use.
      //
      // Only a desktop that ran can be dormant. With no session ever started
      // the loop is retrying a boot that failed, and answering "the desktop is
      // not running, click Refresh" there replaces the real reason (no
      // kwin_wayland, a refused plugin, a distribution refusal) with an
      // instruction that cannot help, and stands the loop down on a failure
      // that might yet clear.
  ...
  /**
   * Boots the session, rebuilding the plugin once if the compositor refuses it.
   *
   * `pluginInstalled()` only asks whether a `SynaraComputerUsePluginVn.so`
   * exists, and that is true of the plugin left behind by whatever KWin this
   * machine ran before its last upgrade. A KWin plugin is a binary module bound
   * to the exact version it was compiled against, so that stale file makes the
   * boot path skip provisioning and then fail on the load the compositor
   * refuses — permanently, because nothing else in the loop ever concludes that
   * the installed file is the problem. The refusal is that conclusion: install
   * a generation built against *this* compositor and boot once more, past the
   * already-current stamp that would otherwise make the reinstall a no-op.
   *
   * Once, and only for this refusal. A second refusal is a real
   * incompatibility, and each retry can cost minutes of compiler time.
   */
  private async startWithPluginRetry(
    sessionOptions: NestedKWinSessionOptions,
  ): Promise<NestedKWinSession> {
    try {
      return await this.startSession(sessionOptions);
    } catch (error) {
      if (!(error instanceof NestedPluginLoadRefusedError)) throw error;
      const reinstalled = await this.provisionOnce(true, true).catch(() => undefined);
      // A failed reinstall means the refusal stands, and its own message is
      // the more useful of the two: it names the compositor's verdict rather
      // than a compiler's.
      if (!reinstalled || reinstalled.requiresRelogin) throw error;
      return await this.startSession(sessionOptions);
    }
  }
  ...
  private async bootSession(): Promise<NestedKWinSession> {
    ...
    if (this.disposing) {
      // The server was told to shut down while this boot was in flight. The
      // session is real and its processes are running, so it is disposed here
