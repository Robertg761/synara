...
/**
 * How computer frames ride the shared fan-out: the wire encoding, the
 * per-subscriber budgets, and which frames are keyframes or codec config so a
 * late or slow subscriber can be resynced from the right one.
 */
const COMPUTER_FRAME_TRANSPORT = {
  encode: (computerId: string, frame: ComputerStreamFrame): Uint8Array =>
    encodeComputerFrame({
      header: {
        computerId,
  ...
  classify: (frame: ComputerStreamFrame) => ({
    keyframe: frame.keyframe,
    codecConfig: frame.codecConfig,
  }),
} as const;
...
interface ThreadComputerRuntimeState {
  version: number;
  /**
   * The newest error explicitly recorded for this thread, or the last desktop
   * refresh's failure. A successful refresh clears it only when nothing was
   * recorded after that refresh started, so an error reported while a publish
   * was already reading the desktop is not erased by the snapshot that follows.
   */
  lastError: string | null;
  /** Bumped by every explicit error record; see `lastError`. */
  errorVersion: number;
...
/** Thread runtime records kept before the least recently touched are evicted. */
export const COMPUTER_THREAD_RECORD_LIMIT = 256;
/**
 * Threads remembered as having had the pane surfaced for them. Larger than the
 * record limit on purpose: the latch has to outlive the record, or a thread
 * evicted between two actions would pull the user back into the pane again.
 */
export const COMPUTER_PANE_SURFACED_LIMIT = 4_096;

/** The single desktop's exclusive owner, and when it last drove it. */
interface DesktopLease {
  readonly threadId: string;
  /**
   * The owner's turn when it last drove the desktop, when the caller said.
   * A release that names a different turn is a late terminal event for an
   * earlier turn and must not take the desktop from the one now running.
   */
...
/** What a caller running agent activity may say about the call. */
export interface AgentActivityOptions {
  readonly signal?: AbortSignal | undefined;
  /** The turn the call belongs to, carried onto the lease it takes or renews. */
...
export class ComputerManager {
  ...
  /**
   * The threads whose agent activity has already asked the UI to open the
   * computer pane, in first-surfaced order. Actions arrive every few seconds,
   * so surfacing is once per thread: repeating the request would emit an event
   * per click and could yank a user who deliberately closed the pane back to
   * it. Kept apart from the thread records so it neither pins a record in
   * memory for ever nor is forgotten when one is evicted.
   */
  private readonly paneSurfacedThreads = new Set<string>();
  /**
  ...
  /** The turn each thread's in-flight agent calls belong to; see `DesktopLease.turnId`. */
  private readonly agentTurnIds = new Map<string, string>();
  ...
  /**
   * Why the frame stream is not attached, while it is not. Kept apart from the
   * per-thread refresh error because the two answer different questions and
   * clear on different events: a window list can be read perfectly well while
   * the compositor refuses the capture stream, and the successful read must not
   * erase the reason the pane is still showing "Connecting". Cleared by the
   * attach that succeeds.
   */
  ...
  constructor(options: ComputerManagerOptions) {
    ...
    this.cursorActivity = new CursorActivity((text) => {
      this.activity = text;
      // Only the owner's snapshot carries the activity text, so only the owner's
      // panel has anything new to show; a badge change is not a reason to
      // repaint every conversation's pane.
      if (this.lease) this.publishCached(this.lease.threadId);
    ...
    this.transport =
      options.transport ??
      new FrameTransport<string, ComputerStreamFrame>(COMPUTER_FRAME_TRANSPORT);
    if (options.backend.onEvent) {
      this.backendUnsubscribe = options.backend.onEvent((event) => {
        if (event.type === "windows-changed") {
          ...
          // A pane that asked for frames while the backend was down was left
          // with a failed attach and nothing to retry it; the reconnect is
          // that retry's cue. Reconciling is idempotent for an attached stream.
          if (event.health.status === "connected") {
  ...
  /**
   * Every accessibility read this class makes, with the tree held to the
   * contract's budget. A backend bounds its own walk, but the contract's
   * ceiling is what the payload is encoded against, and an overflow there
   * would fail a perception read that already succeeded.
   */
  private async readBackendState(
    options: Parameters<ComputerBackend["getState"]>[0],
  ): Promise<ComputerState> {
    const state = await this.backend.getState(options);
    return state.root ? { ...state, root: clampComputerUiTree(state.root) } : state;
  }
  ...
  async moveCursor(
    ...
    return this.withDesktopControl(threadId, async () => {
      const resolved = await this.resolvePointTarget(target);
      await this.revealTarget(resolved);
  ...
  private async resolveScrollPointTarget(target: ComputerTarget): Promise<ResolvedPointTarget> {
    ...
    const state = await this.readBackendState({ includeTree: false });
  ...
  async withAgentActivity<A>(
    ...
    return this.permissions.run(threadId, options.signal, (activeSignal) =>
      this.operations.run(() => this.trackAgentActivity(threadId, action, options), activeSignal),
    );
  }
  ...
  withAgentReadActivity<A>(
    ...
    return this.permissions.run(threadId, options.signal, (activeSignal) =>
      this.operations.read(() => this.trackAgentActivity(threadId, action, options), activeSignal),
    );
  }

  private async trackAgentActivity<A>(
    threadId: string,
    action: () => Promise<A>,
    options: AgentActivityOptions,
  ): Promise<A> {
    ...
    if (options.turnId) this.agentTurnIds.set(owner, options.turnId);
    ...
    try {
      ...
      if (remaining === 0) {
        ...
        this.agentTurnIds.delete(owner);
        if (this.lease?.threadId === owner && this.lease.releaseRequested) {
          // The action's own result is what the caller gets back; a release
          // that could not tidy the seat must never replace it with an error.
          await withoutDesktopCancellation(() =>
            this.releaseDesktopControl(owner).catch(() => undefined),
          );
        } else {
  ...
  private async claimDesktopControl(threadId: string | undefined): Promise<void> {
    ...
    // The running call's turn, or the one the lease already names when this
    // renewal did not say — a claim from outside agent activity (pane-driven
    // tests, direct calls) must not strip the turn a tool call recorded.
    const turnId = this.agentTurnIds.get(owner) ?? (changed ? undefined : held?.turnId);
    this.lease = {
      threadId: owner,
    ...
    if (changed) {
      ...
      // thread starts being. From cache: the handoff changes who holds the
      // desktop, not what is on it, and this runs inside the serialized
      // desktop operation, where a round of backend reads per thread would
      // hold every other caller behind it.
      this.republishAllThreads();
    }
  }
  ...
  setThreadLabel(threadId: string, label: string | null): void {
    ...
    if (trimmed) {
      ...
      for (const id of this.threadLabels.keys()) {
        if (this.threadLabels.size <= COMPUTER_THREAD_RECORD_LIMIT) break;
   ...
   *
   * A release that names a turn applies only to a lease taken in that turn.
   * Terminal turn events are delivered asynchronously, so `turn.completed` for
   * turn 1 can arrive after turn 2 has already taken the desktop back; that
   * late event must not strip the running turn of its hold. A release naming
   * no turn (a dead session, a deleted thread, a permission change) applies
   * whatever the lease says.
   */
  async releaseDesktopControl(
    threadId: string,
    options: { readonly turnId?: string | null | undefined } = {},
  ): Promise<void> {
    const owner = agentThreadId(threadId);
    if (owner === undefined || this.lease?.threadId !== owner) return;
    if (options.turnId && this.lease.turnId !== undefined && options.turnId !== this.lease.turnId) {
      return;
    }
    ...
    // Ownership ends here, before the seat is tidied and whether or not that
    // tidying works: a disconnected compositor must not leave a finished
    // thread owning the desktop until the idle backstop. The next owner resets
    // the seat itself in claimDesktopControl before it can send input, so the
    // reset below is a courtesy to the human, and best effort.
    this.lease = null;
    await this.announceDrivingAgent(null);
    this.republishAllThreads();
    await this.operations
      .run(async () => {
        // Only while nobody has taken over: a claim that was already queued
        // ahead of this reset has aimed the seat for its own owner by now.
        if (this.lease === null) await this.resetInputDelivery();
      })
      .catch(() => undefined);
  }
  ...
  /**
   * Records an error against one thread and shows it. Published from cache
   * rather than by refreshing: a refresh that succeeds would answer "no
   * error", which is not what the caller just reported. The next refresh that
   * starts after this record and succeeds clears it.
   */
  ...
  async handleThreadRemoved(threadId: string): Promise<void> {
    ...
    this.paneSurfacedThreads.delete(threadId);
  ...
  /** The stream is attached, so whatever stopped it last time is over. */
  private clearStreamError(): void {
  ...
  private async resolveSemanticTarget(
    ...
    const state = await this.readBackendState({
      includeTree: true,
  ...
  private surfacePaneForAgent(threadId: string): void {
    ...
    if (this.backendCapabilities.visibleDesktop || this.paneSurfacedThreads.has(threadId)) return;
    this.paneSurfacedThreads.add(threadId);
    for (const id of this.paneSurfacedThreads) {
      if (this.paneSurfacedThreads.size <= COMPUTER_PANE_SURFACED_LIMIT) break;
      this.paneSurfacedThreads.delete(id);
    }
    this.emit({
  ...
  private async publishNow(
    ...
    const errorVersion = state.errorVersion;
    try {
      ...
      // An error recorded while this read was in flight describes something
      // the read did not test; only a read that started after it may clear it.
      if (state.errorVersion === errorVersion) state.lastError = null;
    } catch (error) {
  ...
  private threadRuntime(threadId: string): ThreadComputerRuntimeState {
    ...
    for (const id of this.threads.keys()) {
      if (this.threads.size <= COMPUTER_THREAD_RECORD_LIMIT) break;
      if (
        id === threadId ||
  ...
  private threadSnapshot(threadId: string, state: ThreadComputerRuntimeState): ThreadComputerState {
    return {
      ...
      health: this.backendHealth,
      capabilities: this.backendCapabilities,
      lastError: state.lastError ?? this.streamError,
      provisionable: this.backend.provision !== undefined,
    };
  }
  ...
  /**
   * A stream transition failed. Remembered until an attach succeeds, and shown
   * from cache: refreshing the desktop here would both put a backend round
   * trip on every failure of an outage and, when the read succeeded, erase the
   * very error it was published to show. Debounced, because this can fire per
   * frame or per call during an outage.
   */
  private recordError(error: unknown): void {
    this.streamError = clampComputerMessage(
      errorMessage(error),
      "The computer backend reported an error without a message.",
    );
    if (this.threads.size === 0 || this.disposed) return;
