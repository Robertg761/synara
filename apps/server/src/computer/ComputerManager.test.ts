...
import {
  ...
  COMPUTER_THREAD_RECORD_LIMIT,
  ComputerManager,
  type ComputerManagerOptions,
} from "./ComputerManager.ts";
import {
  coveredCalculatorWindows,
  FAKE_BROWSER_WINDOW,
  FAKE_CALCULATOR_WINDOW,
  FAKE_TERMINAL_WINDOW,
  overlappingWindows,
} from "./computerTestFixtures.ts";
...
/**
 * One manager over a fake backend, with every event it emits recorded and
 * disposal handled after the test. Actions settle instantly: these tests
 * assert on what the manager does, not on how long the desktop gets to repaint.
 */
function makeManager(
  options: Partial<ComputerManagerOptions> & { readonly backend?: FakeComputerBackend } = {},
) {
  const backend = options.backend ?? new FakeComputerBackend();
  const manager = new ComputerManager({ actionSettleMs: 0, ...options, backend });
  ...
  return { backend, manager, events, states };
}
...
describe("ComputerManager and FakeComputerBackend", () => {
  ...
  it("publishes thread snapshots, activity transitions, and backend window events", async () => {
    const { backend, manager } = makeManager({
      backend: new FakeComputerBackend({ now: () => "2026-08-15T00:00:00.000Z" }),
    });
  ...
  it("republishes every thread when backend health changes, without touching the backend", async () => {
    const { backend, manager, states } = makeManager();
  ...
  it("passes a backend's session-lock flag through the health snapshot untouched", async () => {
    const { backend, manager } = makeManager();
    ...
    expect((await manager.getThreadState("thread-a")).health.locked).toBe(true);
    expect((await manager.getStatus()).health.locked).toBe(true);
  });
  ...
  it("answers getStatus without a thread, corrected by live health", async () => {
    ...
    // Nothing has connected the backend yet, and status says so honestly.
    expect(status.health.status).toBe("unavailable");
    ...
    await manager.listWindows();
    expect((await manager.getStatus()).health.status).toBe("connected");
  ...
  it("provisions through the backend and answers with the engaged status", async () => {
    ...
    // The thread snapshot says the same, so a pane can offer "Set up" from it.
    expect((await manager.getThreadState("thread-1")).provisionable).toBe(true);
  ...
  it("refuses to provision a backend with nothing to install", async () => {
    ...
    expect((await manager.getThreadState("thread-1")).provisionable).toBe(false);
  });
  ...
  it("reveals a hover target without changing keyboard aim", async () => {
    const { backend, manager } = makeManager({
      backend: new FakeComputerBackend({ windows: coveredCalculatorWindows() }),
    });
  ...
  it("routes keyboard input to a named window and leaves focus alone without one", async () => {
    ...
    // The one reset is the lease claim, not any keystroke.
    expect(backend.callsFor("resetInputDelivery")).toHaveLength(1);
  ...
  it("carries clipboard text on the shared action result, and refuses it without backend support", async () => {
    ...
    const { manager: withoutClipboard } = makeManager({
  ...
  it("tells the backend which thread is driving, so the agent cursor can name it", async () => {
    ...
    const { manager } = makeManager({ backend: drivable });
  ...
  it("publishes a cursor-activity change to the owning thread only", async () => {
    vi.useFakeTimers();
    try {
      ...
      manager.cursorActivity.setRuntime("thread-a", "Waiting for you");
      await vi.advanceTimersByTimeAsync(100);

      expect(states.map((state) => state.threadId)).toEqual(["thread-a"]);
      expect(states[0]?.activity).toBe("Waiting for you");
    } finally {
      vi.useRealTimers();
    }
  });
  ...
  it("keeps the once-per-thread pane latch across thread record eviction", async () => {
    const { manager, events } = makeManager();
    const openRequests = () =>
      events.filter((event) => event.type === "computer.open-pane-requested").length;
    const total = COMPUTER_THREAD_RECORD_LIMIT + 44;
    for (let index = 0; index < total; index += 1) {
      await manager.click(`thread-${index}`, { x: 10, y: 10 });
      await manager.releaseDesktopControl(`thread-${index}`);
    }
    expect(openRequests()).toBe(total);
    // The records are bounded: the earliest threads have been evicted...
    const threads = (manager as unknown as { threads: Map<string, unknown> }).threads;
    expect(threads.size).toBeLessThanOrEqual(COMPUTER_THREAD_RECORD_LIMIT);
    expect(threads.has("thread-0")).toBe(false);
    // ...and an evicted thread acting again is not surfaced a second time.
    await manager.click("thread-0", { x: 10, y: 10 });
    expect(openRequests()).toBe(total);
  });
  ...
  it("ignores a release naming an earlier turn than the one holding the desktop", async () => {
    const { manager } = makeManager();
    await manager.getThreadState("thread-b");
    await manager.withAgentActivity("thread-a", () => manager.click("thread-a", { x: 1, y: 1 }), {
      turnId: "turn-2",
    });

    // A late turn.completed for turn 1 must not free turn 2's desktop.
    await manager.releaseDesktopControl("thread-a", { turnId: "turn-1" });
    await expect(manager.click("thread-b", { x: 2, y: 2 })).rejects.toMatchObject({
      code: "computer_controlled_by_other_thread",
    });

    // The turn that holds it, and a release naming no turn at all, both end it.
    await manager.releaseDesktopControl("thread-a", { turnId: "turn-2" });
    await expect(manager.click("thread-b", { x: 2, y: 2 })).resolves.toMatchObject({
      action: "computer_click",
    });
    await manager.releaseDesktopControl("thread-b");
    await manager.withAgentActivity("thread-a", () => manager.click("thread-a", { x: 1, y: 1 }), {
      turnId: "turn-3",
    });
    await manager.releaseDesktopControl("thread-a");
  ...
  it("hands the desktop over from cached state, without reading the backend", async () => {
    const { backend, manager, states } = makeManager();
    await manager.listWindows();
    await manager.getThreadState("thread-a");
    await manager.getThreadState("thread-b");
    const reads = () =>
      backend.calls.filter((call) => ["availability", "listWindows"].includes(call.method)).length;

    const before = reads();
    await manager.typeText("thread-a", "hi");
    await manager.releaseDesktopControl("thread-a");
    await manager.typeText("thread-b", "hi");
    expect(reads()).toBe(before);
    // Both panels still learned about every transition.
    const forB = states.filter((state) => state.threadId === "thread-b");
    expect(forB.map((state) => state.controlledByOtherThread)).toEqual(
  ...
  it("resets the seat's input delivery whenever the desktop changes owner", async () => {
    const { backend, manager } = makeManager();
    await manager.typeText("thread-a", "aimed", "fake-calculator");
    expect(backend.callsFor("resetInputDelivery")).toHaveLength(1);
    // The same owner keeps its aim across calls: no reset between them.
    await manager.typeText("thread-a", "more");
    expect(backend.callsFor("resetInputDelivery")).toHaveLength(1);

    // Handing over resets: the next owner's unscoped keystroke must not land
    // in the window the previous owner aimed at.
    await manager.releaseDesktopControl("thread-a");
    expect(backend.callsFor("resetInputDelivery")).toHaveLength(2);
    await manager.typeText("thread-b", "unscoped");
    expect(backend.callsFor("resetInputDelivery")).toHaveLength(3);
    expect(backend.callsFor("clearFocusWindow")).toHaveLength(0);

    // A backend without the full reset falls back to clearing focus.
    const withoutReset = new Proxy(new FakeComputerBackend(), {
      get: (target, property, receiver) =>
        property === "resetInputDelivery" ? undefined : Reflect.get(target, property, receiver),
    }) as FakeComputerBackend;
    const fallback = makeManager({ backend: withoutReset });
    // Window-scoped, so the only clear is the handoff's own.
    await fallback.manager.typeText("thread-a", "aimed", "fake-calculator");
    expect(withoutReset.callsFor("clearFocusWindow")).toHaveLength(1);
  });

  it("releases ownership even when the seat reset fails, and resets again for the next owner", async () => {
    const { backend, manager, states } = makeManager();
    await manager.getThreadState("thread-b");
    await manager.click("thread-a", { x: 10, y: 10 });

    backend.failNext("resetInputDelivery", new Error("Compositor disconnected"));
    // The release itself never fails: ownership has ended whatever the seat
    // reset managed to do.
    await expect(manager.releaseDesktopControl("thread-a")).resolves.toBeUndefined();
    ...
    // The next owner resets the seat before it may send input, so a reset that
    // still fails refuses the input rather than sending it through a seat
    // aimed at the previous owner's window.
    backend.failNext("resetInputDelivery", new Error("Compositor still disconnected"));
  ...
  it("returns the action's result when a deferred release fails to reset the seat", async () => {
    const { backend, manager } = makeManager();
    await manager.getThreadState("thread-b");
    ...
    const inFlight = manager.withAgentActivity("thread-a", async () => {
      ...
      return "the action's own result";
    });
    await started.promise;
    await manager.releaseDesktopControl("thread-a");
    backend.failNext("resetInputDelivery", new Error("Compositor disconnected"));
    finish.resolve();
    await expect(inFlight).resolves.toBe("the action's own result");
    // The failed reset did not keep the lease until the idle backstop.
    await expect(manager.getThreadState("thread-b")).resolves.toMatchObject({
      controlledByOtherThread: false,
    });
    await expect(manager.typeText("thread-b", "hi")).resolves.toMatchObject({
  ...
  it("expires an idle lease as a backstop, but never one whose owner is still acting", async () => {
    let nowMs = 0;
    const { manager } = makeManager({ now: () => nowMs, leaseIdleMs: 1_000 });
    await manager.getThreadState("thread-a");
  ...
  it("counts an agent's in-flight call even when no panel ever asked about it", async () => {
    ...
    let nowMs = 0;
    const { manager } = makeManager({ backend, now: () => nowMs, leaseIdleMs: 1_000 });
  ...
  it("coalesces a burst of window changes into a single publish pass", async () => {
    const { backend, manager } = makeManager({ windowsPublishDebounceMs: 5 });
  ...
  it("runs the synthetic frame attach, publish, and detach loop", async () => {
    ...
    // Attaching opens the tap; the codec config and keyframe arrive on request.
    expect(sink.received).toHaveLength(0);
    await manager.requestKeyframe();
  ...
  it("keeps a stream attach failure on the snapshot until the stream attaches", async () => {
    vi.useFakeTimers();
    try {
      const { backend, manager, states } = makeManager();
      await manager.listWindows();
      await manager.getThreadState("watcher");
      backend.failNext("attachStream", new Error("compositor refused the stream"));
      const unsubscribe = manager.subscribeFrames(new RecordingSink());
      await manager.flushStreamTransitions();
      // The engagement republish refreshed the desktop successfully in the
      // meantime; that must not erase why the pane is still connecting.
      await vi.advanceTimersByTimeAsync(COMPUTER_ERROR_REPUBLISH_DEBOUNCE_MS);
      expect(states.at(-1)).toMatchObject({
        threadId: "watcher",
        lastError: expect.stringContaining("compositor refused the stream"),
      });
      expect((await manager.getThreadState("watcher")).lastError).toContain("refused");

      // The attach that succeeds is what clears it.
      unsubscribe();
      await manager.flushStreamTransitions();
      const resubscribe = manager.subscribeFrames(new RecordingSink());
      await manager.flushStreamTransitions();
      expect(states.at(-1)).toMatchObject({ threadId: "watcher", lastError: null });
      resubscribe();
    } finally {
      vi.useRealTimers();
    }
  ...
  it("reattaches a wanted stream when the backend reports it is connected again", async () => {
    const { backend, manager } = makeManager();
    await manager.listWindows();
    const sink = new RecordingSink();
    backend.failNext("attachStream", new Error("compositor unavailable"));
    const unsubscribe = manager.subscribeFrames(sink);
    await manager.flushStreamTransitions();
    expect(backend.callsFor("attachStream")).toHaveLength(1);
    ...
    // A stream that is already attached is left alone by further reconnects.
    backend.emitHealthChanged({
      status: "connected",
      consecutiveFailures: 0,
      reconnects: 2,
  ...
  it("does not let an in-flight refresh erase an error recorded after it started", async () => {
    const held = deferred();
    ...
    const { manager, states } = makeManager({ backend: new DelayedBackend() });
    const refreshing = manager.getThreadState("reported");
    await entered.promise;
    await manager.recordThreadError("reported", "the tool call failed");
    expect(states.at(-1)?.lastError).toBe("the tool call failed");
    held.resolve();
    expect((await refreshing).lastError).toBe("the tool call failed");
    // A refresh that starts after the record, and succeeds, clears it.
    expect((await manager.getThreadState("reported")).lastError).toBeNull();
  });
  ...
  it("holds a backend's oversized accessibility tree to the contract budget", async () => {
    let root: ComputerUiNode = {
      role: "leaf",
      label: null,
      value: null,
      description: null,
    ...
    for (let depth = 1; depth < COMPUTER_UI_TREE_MAX_DEPTH + 10; depth += 1) {
      const child = root;
      root = {
        role: "group",
        label: null,
    ...
    const { manager } = makeManager({ backend: new FakeComputerBackend({ root }) });
    const state = await manager.getState({ includeTree: true });
    let depth = 0;
    for (let node = state.root; node; node = node.children[0]) depth += 1;
    expect(depth).toBe(COMPUTER_UI_TREE_MAX_DEPTH);
    expect(() => Schema.decodeUnknownSync(ComputerState as never)(state)).not.toThrow();
  });
  ...
  it("captures the focused window, and the workspace when nothing capturable has focus", async () => {
    ...
    backend.emitWindowsChanged([
      { ...FAKE_TERMINAL_WINDOW, minimized: true, visible: false, stackingIndex: 1 },
      { ...FAKE_CALCULATOR_WINDOW, stackingIndex: 0 },
    ]);
  ...
  it("resolves overlapping point candidates by stacking order and refuses to guess without one", async () => {
    ...
    backend.emitWindowsChanged(overlappingWindows(true));
    ...
    backend.emitWindowsChanged(overlappingWindows(false));
  ...
  it("engages the backend when the pane attaches or the human drives it", async () => {
    ...
    const { manager: paneManager } = makeManager({ backend: paneBackend });
    const sink = new RecordingSink();
    ...
    const { manager: inputManager } = makeManager({ backend: inputBackend });
  ...
  it("scrolls inside the desktop operation: one window read, and paused input refused", async () => {
    const { backend, manager } = calibratedScrollFixture([336, 64]);
    await manager.getThreadState("thread-1");
    await manager.click("thread-1", { x: 1_100, y: 200 });

    // The window under the cursor, the focus fallback, and the pre-clear focus
    // all come from one cached listing rather than three uncached ones.
    const readsBefore = backend.callsFor("listWindows").length;
    await manager.scrollCalibrated("thread-1", null, 0, 400, { observe: true });
    expect(backend.callsFor("listWindows").length - readsBefore).toBe(1);

    // A thread whose input is paused cannot scroll around the pause.
    backend.failNext(
      "typeText",
      new ComputerBackendError("Return to the target window.", {
        inputPause: { windowId: "fake-calculator", message: "Return to the target window." },
      }),
    );
    await expect(manager.typeText("thread-1", "x")).rejects.toHaveProperty("inputPause");
    const scrolls = backend.callsFor("scroll").length;
    await expect(
      manager.scrollCalibrated("thread-1", null, 0, 40, { observe: false }),
    ).rejects.toHaveProperty("inputPause");
    expect(backend.callsFor("scroll")).toHaveLength(scrolls);
  });
