...
export interface ComputerBackend {
  ...
  /**
   * Forget everything one driver left behind before the next one starts:
   * the explicit focus/aim target, any cached pointer or keyboard "enter"
   * bookkeeping a direct-injection path keeps, held buttons and keys, and
   * shared client objects handed back to the human seat. Called whenever the
   * desktop lease changes owner. Optional because a backend that keeps no
   * such state has nothing beyond `clearFocusWindow` to do, which is what the
   * manager falls back to.
   */
