...
const make = Effect.gen(function* () {
  ...
  const releaseDesktopControl = (event: ProviderRuntimeEvent) =>
    Effect.promise(() =>
      computerService.manager.releaseDesktopControl(event.threadId, {
        // A dead session ends every turn's hold; a terminal turn event ends
        // only the turn it names, so a late one cannot take the desktop from
        // the turn now running.
        turnId: event.type === "session.exited" ? undefined : event.turnId,
      }),
    ).pipe(
