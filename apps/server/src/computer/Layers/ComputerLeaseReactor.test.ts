...
function event(
  ...
  return {
    type,
    eventId: EventId.makeUnsafe(`event-${type}-${threadId}-${turnId}`),
    provider: "codex",
...
describe("ComputerLeaseReactor", () => {
  ...
  it("ignores a late turn end for an earlier turn of the owning thread", async () => {
    const manager = new ComputerManager({ backend: new FakeComputerBackend() });
    try {
      await manager.withAgentActivity(OWNER, () => manager.click(OWNER, { x: 5, y: 5 }), {
        turnId: "turn-2",
      });

      await withReactor(
        { manager, events: [event("turn.completed", OWNER, "turn-1")] },
        async () => {
          // Give the reactor its chance to (wrongly) release, then prove it did not.
          await waitUntil(
            async () => (await manager.getThreadState(OTHER)).controlledByOtherThread,
          );
      ...
      // The turn that actually holds it frees it; a dead session frees it too.
      await withReactor(
        { manager, events: [event("turn.completed", OWNER, "turn-2")] },
        async () => {
          await waitUntil(
            async () => !(await manager.getThreadState(OTHER)).controlledByOtherThread,
          );
      ...
      await withReactor(
        { manager, events: [event("session.exited", OWNER, "turn-1")] },
        async () => {
          await waitUntil(
            async () => !(await manager.getThreadState(OTHER)).controlledByOtherThread,
          );
