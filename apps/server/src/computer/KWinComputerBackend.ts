...
import { readHostKwinVersion } from "./provisioning/hostToolchain.ts";
import {
  commandOnPath,
...
const SESSION_LOCKED_MESSAGE =
  "the session is locked or inactive, so capture and input are refused until it is unlocked.";
...
export interface KWinComputerBackendOptions {
  ...
  /**
   * The KWin version installed on disk (see `readHostKwinVersion`), which is
...
export class KWinComputerBackend implements ComputerBackend {
  ...
  /** The plugin's dedicated seat, or the nested compositor's own: see the interface. */
  ...
  /**
   * Aborted by `dispose`: cancels an install in flight and cuts short a
   * connect ladder's backoff, both of which `dispose` would otherwise wait on.
   */
  ...
  /**
   * One live health read, recorded the same way whoever asks: the establishing
   * read and the input-readiness check both go through here, so a pause can
   * only clear on a plugin the establishing read would also call healthy.
   *
   * Locked is still available — the desktop comes back when the human unlocks
   * — but a pane asking why nothing moves deserves the answer the next action
   * would refuse with, and health is the only place to put it that the
   * contract carries.
   */
  private async readPluginHealth(plugin: KWinComputerPluginApi): Promise<KWinHealth> {
    ...
    this.publishHealth();
    if (health.locked) {
      this.recordHealthFailure(this.sessionLockedError(SESSION_LOCKED_MESSAGE));
      this.publishHealth();
    }
  ...
  async resetInputDelivery(): Promise<void> {
    ...
    try {
      await this.pluginValue(() => plugin.resetInputDelivery());
    } catch (error) {
      if (!isUnknownMethodDbusError(error)) throw error;
      await this.pluginValue(() => plugin.clearFocusWindow());
    }
  }
  ...
  /**
   * Refused rather than approximated: AT-SPI exposes no settable selection
   * this backend reads back, and the contract forbids standing in for it with
   * a triple-click or a select-all the application would read differently.
   */
  async selectText(target: ComputerResolvedTarget, _range: ComputerTextRange): Promise<never> {
    throw new ComputerBackendError(
      `Selecting a text range in the ${target.node.role} ${JSON.stringify(target.node.label ?? "")} ` +
        "is not supported on this desktop: the accessibility layer exposes no selection to set. " +
        "Use set_value to replace the field, or click and keyboard navigation to place the caret.",
      { rejectedOperation: "selectText" },
    );
  }

  /**
   * Whether the exact window can take input now, answered without focusing,
   * raising, or sending anything. The one way a latched input pause clears:
   * the plugin reports the session unlocked and the window is still there.
   */
  async checkInputReady(windowId: string): Promise<void> {
    try {
      ...
      const health = await this.readPluginHealth(plugin);
      if (health.releaseShortcut === null) {
        throw new ComputerBackendError(this.noReleaseShortcutMessage());
      }
      if (health.locked) throw this.sessionLockedError(SESSION_LOCKED_MESSAGE, undefined, windowId);
      const [windows] = await this.readWindows();
      if (!windows.some((window) => window.id === windowId)) {
        throw new ComputerBackendError(
          `Window ${JSON.stringify(windowId)} is no longer on the desktop, so it cannot take input.`,
          { rejectedOperation: "checkInputReady" },
        );
      }
    } catch (error) {
      // The same classification every plugin call gets: a dead connection
      // schedules its reconnect here rather than leaving a pause to clear on
      // a plugin nothing is talking to.
      throw this.reportPluginFailure(error);
    }
  }

  /**
   * The emergency stop: everything the seat holds goes back to the human at
   * once. Only a session that is up has anything to release — this must not
   * connect or start one to stop it.
   */
  ...
  private async connectWithBackoff(automatic: boolean): Promise<KWinComputerPluginApi> {
    ...
    for (let attempt = 0; attempt < 3; attempt += 1) {
      // Disposal ends the ladder. `dispose` waits for this very promise, so a
      // rung after it would hold the teardown hostage to a backoff and open a
      // fresh bus connection — or load a plugin — for a backend nobody owns.
      if (this.disposed) throw this.disposedError();
      try {
        ...
        // one thing it must never do. A backend disposed mid-attempt leaves
        // the connection to `dispose`, which closes it.
        if (
          this.disposed ||
        ...
        if (attempt < 2)
          await this.backoffUntilDisposed(KWIN_RECONNECT_BASE_DELAY_MS * 2 ** attempt);
      }
    }
    throw new ComputerBackendError(
  ...
  /**
   * A backoff rung's wait, cut short by `dispose`: the wait is only worth
   * finishing for a backend that will try again, and `dispose` waits for the
   * connect that is sleeping here.
   */
  private async backoffUntilDisposed(milliseconds: number): Promise<void> {
    const signal = this.disposal.signal;
    if (signal.aborted) return;
    const disposed = Promise.withResolvers<void>();
    const onDisposed = () => disposed.resolve();
    signal.addEventListener("abort", onDisposed, { once: true });
    try {
      await Promise.race([this.sleep(milliseconds), disposed.promise]);
    } finally {
      signal.removeEventListener("abort", onDisposed);
    }
  }

  private disposedError(): ComputerBackendError {
    return new ComputerBackendError(`${this.integrationName} computer backend is disposed.`);
  }
  ...
  /**
   * The lock refusal as the manager understands it: an input pause it latches
   * for the thread until `checkInputReady` sees the desktop back, so the
   * model stops and hands back instead of retrying against a lock screen.
   * Observation stays open — capture is what the plugin refuses, and the
   * manager reads the pause off the same error.
   */
  private sessionLockedError(
    detail: string,
    cause?: unknown,
    // The window the pause is about, so a scoped observation of it can clear
    // the pause: the one the caller named, else the one the seat was aimed at.
    // A pause naming no window clears only through an unscoped observation.
    windowId: string | undefined = this.lastAimedWindowId,
  ): ComputerBackendError {
    const message = `${SESSION_LOCKED_REFUSAL}: ${detail}`;
    return new ComputerBackendError(message, {
      retryable: true,
      inputPause: {
        ...(windowId === undefined ? {} : { windowId }),
        message: clampComputerMessage(message, SESSION_LOCKED_MESSAGE),
      },
      ...(cause === undefined ? {} : { cause }),
    });
  }
  ...
  /**
   * Announces the transition into a locked session once per lock, so consent
   * granted before the lock does not silently authorize the desktop after it.
   * The plugin reports the lock on every health read and refusal, which is
   * why this remembers the last state rather than emitting on each.
   */
  private noteSessionLock(locked: boolean): void {
    if (locked === this.sessionLocked) return;
    this.sessionLocked = locked;
    if (locked) this.emit({ type: "desktop-interrupted", pauses: ["screen-lock"] });
  }
  ...
  private reportPluginFailure(error: unknown): ComputerBackendError {
    ...
    if (dbusErrorType(error) === SESSION_LOCKED_ERROR_TYPE) {
      this.noteSessionLock(true);
      return this.sessionLockedError(dbusErrorText(error), error);
    }
