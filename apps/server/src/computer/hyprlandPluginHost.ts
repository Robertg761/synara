...
export async function createSessionHyprlandComputerDbus(
  ...
  return {
    ...
    listLoadedPluginIds: async () => {
      ...
      const ids: string[] = [];
      const synaraCount = plugins.filter((plugin) => plugin.name === HYPRLAND_PLUGIN_NAME).length;
      let resolvedSynara = false;
      for (const plugin of plugins) {
        if (plugin.name !== HYPRLAND_PLUGIN_NAME) {
          ids.push(plugin.name);
          continue;
        }
        // Only one of several loaded generations owns the bus name, and
        // healthJson answers for that one alone — so asking it once per entry
        // returns the same id twice, the engine unloads that id twice, and the
        // other generation is never unloaded at all while it keeps the service
        // name away from the build that just replaced it. When more than one is
        // loaded the honest answer is "an unidentifiable Synara build", whose
        // unload sweeps by path instead.
        if (synaraCount > 1) {
          if (!resolvedSynara) {
            resolvedSynara = true;
            ids.push(HYPRLAND_FALLBACK_PLUGIN_ID);
          }
          continue;
        }
    ...
    unloadPlugin: async (pluginId) => {
      ...
      if (pluginId !== HYPRLAND_FALLBACK_PLUGIN_ID && (known || fileExists(pathForId(pluginId)))) {
        const unloaded = await unloadHyprlandPlugin(run, pathForId(pluginId));
        if (unloaded) loadedPaths.delete(pluginId);
        return unloaded;
      }
      // The id stands for "some Synara build whose path this adapter cannot
      // name" — an older build that never answered with its module path, or
      // several generations loaded at once. Every path that could be it gets
      // tried: the generations on disk now, plus every path this adapter has
      // ever loaded or been told about, because provisioning deletes
      // superseded files while Hyprland still has them mapped.
      const candidates = new Set<string>(loadedPaths.values());
      for (const name of await listDirectory()) {
        if (INSTALLED_PLUGIN_FILE.test(name)) candidates.add(join(options.pluginDirectory, name));
      }
      let any = false;
      for (const path of candidates) {
        if (await unloadHyprlandPlugin(run, path)) {
          any = true;
          for (const [id, loaded] of loadedPaths) {
            if (loaded === path) loadedPaths.delete(id);
          }
        }
      }
      return any;
    ...
    // Both are the session bus's, not Hyprland's: a stale plugin generation and
    // a plugin whose event loop has stopped answering look exactly the same on
    // either compositor, and the engine only learns about them through these.
