 ...
 * `directory` is `/tmp` in production, where the plugin looks; tests point it
 * at a scratch directory.
 */
export async function createComputerSessionAuth(busId: string, directory = "/tmp") {
  if (!/^[a-zA-Z0-9_-]{1,128}$/.test(busId)) throw new Error("Invalid D-Bus session identifier.");
  const path = join(directory, `synara-computer-use-${process.getuid!()}-${busId}.token`);
