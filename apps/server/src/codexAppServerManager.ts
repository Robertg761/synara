...
export class CodexAppServerManager extends EventEmitter<CodexAppServerManagerEvents> {
  ...
  async respondToRequest(
    ...
    // changes included — so only the kinds `approvalSessionGrant` calls
    // session-policy may set it. A tool call keeps its own, properly scoped
    // channel: `acceptForSession` on an MCP tool request rides back as
    // `_meta.persist: "session"`, which is the persistence Codex itself
    // advertised, not a blanket grant.
  ...
  private attachProcessListeners(context: CodexSessionContext): void {
    ...
    const onStdoutData = (chunk: Buffer) => {
      ...
      try {
        // Lines the chunk completed are handled before its failures are judged.
        // A dropped line used to take its whole chunk with it, so a response
        // that had already been decoded never reached the turn waiting on it.
        const { frames, errors } = context.stdoutFramer.pushRecords(chunk);
        for (const line of frames) {
          this.handleStdoutLine(context, line);
        }
        const fatal = errors.find(isFatalCodexLineError);
