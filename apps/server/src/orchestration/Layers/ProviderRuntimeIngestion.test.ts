...
describe("ProviderRuntimeIngestion", () => {
  ...
  it("settles a pending user-input request when its turn ends without a session restart", async () => {
    ...
    // Drained to quiescence first: a count taken the instant the first
    // settlement lands cannot see a duplicate arriving a moment later.
    await harness.drain();
  ...
  it("settles a finished turn's request even when a concurrent turn owns the lifecycle", async () => {
    const harness = await createHarness();

    // Two overlapping sends. The first turn.started binds the thread
    // lifecycle, so the second turn's terminal event is classified
    // `conflicts-with-active-turn` and applies nothing to the thread — but its
    // provider callback is gone all the same, and no later event names that
    // turn again.
    for (const turnId of ["turn-lifecycle-owner", "turn-conflicting"]) {
      harness.emit({
        type: "turn.started",
        ...
        lifecycleGeneration: "generation-conflicting",
      });
    }
    emitPendingUserInputRequest(harness, {
      eventId: "evt-user-input-requested-lifecycle-owner",
      requestId: "req-lifecycle-owner",
      turnId: "turn-lifecycle-owner",
      lifecycleGeneration: "generation-conflicting",
    });
    emitPendingUserInputRequest(harness, {
      eventId: "evt-user-input-requested-conflicting",
      requestId: "req-conflicting",
    ...
    harness.emit({
      type: "turn.completed",
      eventId: asEventId("evt-turn-completed-conflicting"),
      provider: "codex",
      createdAt: new Date().toISOString(),
      threadId: asThreadId("thread-1"),
      turnId: asTurnId("turn-conflicting"),
      lifecycleGeneration: "generation-conflicting",
      payload: { state: "interrupted" },
    });
    ...
    const settledThread = await waitForProjectedThread(
      harness.readProjectedThread,
      (thread) => pendingInteractionStatus(thread, "req-conflicting") === "uncertain",
    );
    // The turn that still owns the lifecycle keeps its answerable request.
    expect(pendingInteractionStatus(settledThread, "req-lifecycle-owner")).toBe("pending");
    const failures = userInputFailureActivities(settledThread);
    expect(failures).toHaveLength(1);
    expect(failures[0]?.payload).toMatchObject({ requestId: "req-conflicting" });
  });

  it("settles a turnless Claude approval when its last turn ends", async () => {
    const harness = await createHarness();
    const base = {
      provider: "claudeAgent" as const,
      threadId: asThreadId("thread-1"),
      createdAt: new Date().toISOString(),
      lifecycleGeneration: "generation-claude-turnless",
    };

    harness.emit({
      ...base,
      type: "turn.started",
      eventId: asEventId("evt-turn-started-claude-turnless"),
      turnId: asTurnId("turn-claude-turnless"),
    });
    // Claude's `canUseTool` binds no turn when there is no live turn state, and
    // the adapter only settles its own rows by turn id — so nothing on the
    // provider side can ever close this one.
    harness.emit({
      ...base,
      type: "request.opened",
      eventId: asEventId("evt-request-opened-claude-turnless"),
      requestId: ApprovalRequestId.makeUnsafe("req-claude-turnless"),
      payload: { requestType: "tool_approval", detail: "An MCP tool wants to run" },
    });
    await waitForProjectedThread(
      harness.readProjectedThread,
      (thread) => pendingInteractionStatus(thread, "req-claude-turnless") === "pending",
    );

    harness.emit({
      ...base,
      type: "turn.completed",
      eventId: asEventId("evt-turn-completed-claude-turnless"),
      turnId: asTurnId("turn-claude-turnless"),
      payload: { state: "completed" },
    });
    ...
    const settledThread = await waitForProjectedThread(
      harness.readProjectedThread,
      (thread) => pendingInteractionStatus(thread, "req-claude-turnless") === "uncertain",
    );
    expect(approvalFailureActivities(settledThread)).toHaveLength(1);
  });

  it("leaves turnless rows open when a terminal event names no lifecycle generation", async () => {
    const harness = await createHarness();

    harness.emit({
      type: "turn.started",
      eventId: asEventId("evt-turn-started-turnless-no-generation"),
      provider: "codex",
      createdAt: new Date().toISOString(),
      threadId: asThreadId("thread-1"),
      turnId: asTurnId("turn-turnless-no-generation"),
      lifecycleGeneration: "generation-turnless-no-generation",
    });
    emitTurnlessPendingApprovalRequest(harness, {
      eventId: "evt-request-opened-turnless-no-generation",
      requestId: "req-turnless-no-generation",
      lifecycleGeneration: "generation-turnless-no-generation",
    });
    await waitForProjectedThread(
      harness.readProjectedThread,
      (thread) => pendingInteractionStatus(thread, "req-turnless-no-generation") === "pending",
    );

    // Without a generation there is nothing to match the row against, so this
    // event cannot prove the row belongs to the runtime that just ended.
    harness.emit({
      type: "turn.completed",
      eventId: asEventId("evt-turn-completed-turnless-no-generation"),
      provider: "codex",
      createdAt: new Date().toISOString(),
      threadId: asThreadId("thread-1"),
      turnId: asTurnId("turn-turnless-no-generation"),
    ...
    expect(pendingInteractionStatus(untouchedThread, "req-turnless-no-generation")).toBe("pending");
