...
function planStalePendingRequestCommands(input: {
  ...
  /** This thread's unsettled rows only. */
  ...
  for (const row of input.pendingInteractions) {
    if (!isUnsettledPendingInteraction(row)) {
      continue;
    }
...
export function planRestartTurnReconciliation(input: {
  ...
  // Grouped once: every thread would otherwise rescan the whole unsettled set.
  const pendingInteractionsByThread = Arr.groupBy(
  ...
  for (const thread of input.threads) {
    ...
    commands.push(
      ...planStalePendingRequestCommands({
        thread,
        pendingInteractions: pendingInteractionsByThread[thread.id] ?? [],
        now: input.now,
      }),
    );
...
> = Effect.gen(function* () {
  ...
  const reconcilableThreads = yield* Effect.forEach(
    ...
    (thread) => {
      // No pending-interaction fallback here: the planner already receives the
      // durable rows and settles them on the fallback path.
