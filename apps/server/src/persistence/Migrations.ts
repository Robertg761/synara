...
import Migration0099 from "./Migrations/099_AuthSessionRenewalPolicy.ts";
...
export const migrationEntries = [
  ...
  [99, "AuthSessionRenewalPolicy", Migration0099],
] as const;
...
export const MIGRATION_LINEAGE_ALIASES: readonly MigrationLineageAlias[] = [
  ...
  {
    // Canary builds recorded the session-renewal migration at 97 before
    // upstream released `ProjectionThreadsSidechatLifecycle` under that ID.
    // The slot must re-run (97 is now a different migration), and 99 is
    // idempotent (`columnExists` guard), so removing the row is safe.
    historicalId: 97,
    historicalName: "AuthSessionRenewalPolicy",
    currentId: 99,
