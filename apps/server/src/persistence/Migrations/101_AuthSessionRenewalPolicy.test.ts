...
import AuthSessionRenewalPolicy from "./101_AuthSessionRenewalPolicy.ts";
...
describe("101_AuthSessionRenewalPolicy", () => {
  it.effect("upgrades a database that recorded the Android auth migration at 97", () =>
    Effect.gen(function* () {
      const sql = yield* SqlClient.SqlClient;
      yield* runMigrations({ toMigrationInclusive: 96 });
      yield* insertLegacySession(sql, {
        sessionId: "existing-fixed",
        issuedAt: "2026-01-01T00:00:00.000Z",
        expiresAt: "2026-01-01T01:00:00.000Z",
      });
      yield* AuthSessionRenewalPolicy;
      yield* sql`
        INSERT INTO effect_sql_migrations (migration_id, name)
        VALUES (97, 'AuthSessionRenewalPolicy')
      `;

      const executed = yield* runMigrations();
      assert.deepStrictEqual(executed.map(([id]) => id), [97, 98, 99, 100, 101]);
      const policies = yield* readPolicies(sql);
      assert.strictEqual(policies.get("existing-fixed"), "fixed");
  ...
  it.effect("propagates schema failures and leaves migration 101 retryable", () =>
    Effect.gen(function* () {
      const sql = yield* SqlClient.SqlClient;
      yield* runMigrations({ toMigrationInclusive: 89 });
      yield* sql`DROP TABLE auth_sessions`;

      const exit = yield* Effect.exit(runMigrations({ toMigrationInclusive: 101 }));
