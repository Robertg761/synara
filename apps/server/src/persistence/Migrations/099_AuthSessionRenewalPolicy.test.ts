...
describe("099_AuthSessionRenewalPolicy", () => {
  ...
  it.effect("propagates schema failures and leaves migration 99 retryable", () =>
    Effect.gen(function* () {
      const sql = yield* SqlClient.SqlClient;
      yield* runMigrations({ toMigrationInclusive: 98 });
      yield* sql`DROP TABLE auth_sessions`;

      const exit = yield* Effect.exit(runMigrations({ toMigrationInclusive: 99 }));
      assert.isTrue(Exit.isFailure(exit));

      const tracker = yield* sql<{ readonly count: number }>`
        SELECT COUNT(*) AS count
        FROM effect_sql_migrations
        WHERE migration_id = 99
      `;
