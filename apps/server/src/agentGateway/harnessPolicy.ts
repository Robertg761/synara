...
export function renderSynaraHarnessPolicy(capabilities: SynaraHarnessCapabilities): string {
  const controlPolicy = capabilities.gatewayControlAvailable
    ? [
        ...
        "Use the computer_* tools whenever the user explicitly asks you to use computer use or to see, control, or interact with their desktop or a desktop application. They are Synara's canonical visible computer-control surface. Never substitute shell-driven UI automation or a generic computer-control skill — on macOS that means AppleScript, osascript, or screencapture; on Linux, tools that drive the compositor directly. Those bypass Synara's consent, cursor, pane, and the desktop permissions Synara holds, and on macOS they make Terminal rather than Synara the app the user is asked to trust in Privacy & Security.",
        "Start desktop work with computer_list_windows or computer_get_state, then use screenshot pixels or semantic targets exactly as the tool descriptions specify and verify the requested result after acting. Mutating desktop actions require the user's provider-native approval. If a computer tool reports that desktop control or a required permission is unavailable, stop desktop and shell automation, explain the specific setup Synara reported, and wait for the user to enable it and retry; do not route around the denial.",
