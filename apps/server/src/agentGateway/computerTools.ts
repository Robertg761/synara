...
import { COMPUTER_SEMANTIC_ACTIONS } from "../computer/inputVocabulary.ts";
...
interface ComputerToolAnnotations {
...
/**
 * Every computer tool's annotations, and with them the one fact three things
 * used to each decide for themselves: whether the tool is read-only. The MCP
 * annotations a provider reads, the manager path a call takes (read activity
 * or the exclusive desktop lease), and the approval gate for providers without
 * a native one all derive from `readOnlyHint` here, so a new tool cannot be
 * advertised as read-only to one of them and mutating to another.
 */
const COMPUTER_TOOL_ANNOTATIONS: Readonly<Record<string, ComputerToolAnnotations>> = {
  computer_list_windows: { title: "List computer windows", ...READ_ONLY_TOOL_ANNOTATIONS },
  computer_get_state: { title: "Get computer state", ...READ_ONLY_TOOL_ANNOTATIONS },
  computer_screenshot: { title: "Capture computer screenshot", ...READ_ONLY_TOOL_ANNOTATIONS },
  computer_get_screen_size: { title: "Get screen size", ...READ_ONLY_TOOL_ANNOTATIONS },
  computer_wait: { title: "Wait", ...READ_ONLY_TOOL_ANNOTATIONS },
  // The one read that is not read-only, on purpose: the clipboard is the
  // human's, and it can hold something they copied privately — a password
  // manager entry, a token — that is not otherwise visible to the agent.
  // Providers auto-approve on readOnlyHint, so this must go through approval
  // and take the lease like a mutation. It mutates nothing, hence
  ...
  computer_launch_app: { title: "Launch computer app", ...WRITE_TOOL_ANNOTATIONS },
  computer_click: { title: "Click", ...WRITE_TOOL_ANNOTATIONS },
  computer_double_click: { title: "Double click", ...WRITE_TOOL_ANNOTATIONS },
  computer_triple_click: { title: "Triple click", ...WRITE_TOOL_ANNOTATIONS },
  computer_right_click: { title: "Right click", ...WRITE_TOOL_ANNOTATIONS },
  // A hover is approval-gated because the manager's revealTarget can raise
  // the target window before the pointer moves: the human's stacking order
  // changes, which is exactly what approval exists to cover.
  computer_move_cursor: { title: "Move cursor", ...WRITE_TOOL_ANNOTATIONS },
  computer_drag: { title: "Drag", ...WRITE_TOOL_ANNOTATIONS },
  computer_scroll: { title: "Scroll", ...WRITE_TOOL_ANNOTATIONS },
  computer_type_text: { title: "Type text", ...WRITE_TOOL_ANNOTATIONS },
  computer_press_key: { title: "Press key", ...WRITE_TOOL_ANNOTATIONS },
  computer_hotkey: { title: "Press hotkey", ...WRITE_TOOL_ANNOTATIONS },
  computer_write_clipboard: { title: "Write computer clipboard", ...WRITE_TOOL_ANNOTATIONS },
  // The only tool whose whole effect is on what the human sees on their own
  // screen, which is exactly why it is gated.
  computer_activate_window: { title: "Activate window", ...WRITE_TOOL_ANNOTATIONS },
  computer_set_value: { title: "Set computer value", ...WRITE_TOOL_ANNOTATIONS },
  computer_perform_action: { title: "Perform computer action", ...WRITE_TOOL_ANNOTATIONS },
};

function computerToolAnnotations(name: string): ComputerToolAnnotations {
  const annotations = COMPUTER_TOOL_ANNOTATIONS[name];
  if (!annotations) throw new Error(`Computer tool ${name} has no annotations registered.`);
  return annotations;
}

/** Whether a computer tool is a mutation or a private read: everything not read-only. */
export function computerToolRequiresApproval(name: string): boolean {
  return computerToolAnnotations(name).readOnlyHint !== true;
}

/** The approval-gated tools, derived from the annotations rather than listed again. */
export const COMPUTER_APPROVAL_REQUIRED_TOOLS: ReadonlySet<string> = new Set(
  Object.keys(COMPUTER_TOOL_ANNOTATIONS).filter(computerToolRequiresApproval),
);
...
export function makeAgentGatewayComputerTools(
      ...
      Effect.tryPromise({
        try: async (abortSignal) => {
          if (
            ...
            const outcome = await options.authorizeAction(name, args, context, abortSignal);
            if (outcome.decision !== "accept") {
              return {
                result: mcpToolResultError(
                  outcome.reason === undefined
                    ? "Computer action was denied or cancelled; no input was sent."
                    : `${outcome.reason} No input was sent.`,
                ),
                signal: undefined,
              };
            }
          ...
          const readOnly = !computerToolRequiresApproval(name);
          ...
          const value = await withActivity(
            ...
            { signal: abortSignal, turnId: context.callerTurnId },
          );
  ...
  const actionEntry = (
    ...
    definition: {
      ...
      description,
      inputSchema,
      annotations: computerToolAnnotations(name),
    },
  ...
  return [
    {
      ...
      definition: {
        ...
        annotations: computerToolAnnotations("computer_list_windows"),
      },
    ...
    },
    {
      ...
      definition: {
        ...
        annotations: computerToolAnnotations("computer_get_state"),
      },
    ...
    },
    {
      ...
      definition: {
        ...
        annotations: computerToolAnnotations("computer_screenshot"),
      },
    ...
    },
    {
      ...
      definition: {
        ...
        inputSchema: { type: "object", properties: {}, additionalProperties: false },
        annotations: computerToolAnnotations("computer_get_screen_size"),
      },
    ...
    },
    {
      ...
      definition: {
        ...
        annotations: computerToolAnnotations("computer_wait"),
      },
    ...
    },
    {
      ...
      definition: {
        ...
        inputSchema: { type: "object", properties: {}, additionalProperties: false },
        annotations: computerToolAnnotations("computer_read_clipboard"),
      },
...
function semanticActionNames(dialect: ComputerAgentDialect): readonly string[] {
  return COMPUTER_SEMANTIC_ACTIONS[dialect];
}
