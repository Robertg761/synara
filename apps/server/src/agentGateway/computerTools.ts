 ...
 */
export function computerToolInstructions(input: {
  /** See `ComputerBackend.deliversToNamedWindowRegardlessOfStacking`. */
  readonly deliversWithoutRaising: boolean;
}): string {
  return [
    "## Synara computer use",
    "",
    ...
    "",
    "### Aiming the keyboard",
    keyboardTargetNote(input.deliversWithoutRaising),
    "",
...
function keyboardTargetNote(deliversWithoutRaising: boolean): string {
  return deliversWithoutRaising
    ? "Keys go to whichever window holds the agent seat's keyboard focus, and a keyboard action needs a target: click into the window you mean first, or pass window_id, which delivers to that window regardless of what covers it and without bringing it to the front. computer_move_cursor does not count — a hover moves the agent's overlay and aims nothing. With nothing aimed, the action is refused and nothing is typed — there is no \"whatever is in front\" fallback, because that fallback used to type the agent's text into the human's own document. The aim is re-checked on every call, so a window that has closed since you aimed at it is refused too, rather than typing into whatever inherited its process id. That refusal is not transient: aim first, then send the input again rather than retrying it unchanged."
    : "Keys go to whichever window holds the agent seat's keyboard focus. Pass window_id to raise and focus a specific window first — recommended whenever more than one window is open, since focus otherwise sits wherever the last action left it. computer_move_cursor does not aim the keyboard; only a click, a drag, a scroll, or an explicit window_id does.";
}
...
export function makeAgentGatewayComputerTools(
  ...
  return [
    ...
    actionEntry(
      "computer_activate_window",
      "Activate window",
      'Bring one window forward on the user\'s screen. This is the only computer tool whose whole effect is on what the person sitting at the machine sees, so it is not a routine prelude to acting: input aimed with window_id already reaches its window without moving anything, and activating a window interrupts whatever the human was looking at. Use it in exactly two cases — the user asked for a window to be brought forward, or an input came back with delivery.verified "unconfirmed", you retried once, and computer_list_windows shows the target is not the active window (a few applications drop shortcuts sent to an inactive window). A desktop that cannot restack refuses rather than reporting a move that did not happen, and the window is raised without activating a different application than the one that owns it. It returns no screenshot; look with computer_screenshot or computer_get_state afterwards.',
      {
        type: "object",
        properties: {
          window_id: {
