...
describe("agent gateway computer tools", () => {
  ...
  it("exposes the full Phase 1 surface behind computer:control", async () => {
    ...
    // A hover is gated because the manager's revealTarget can raise the target
    // window before the pointer moves.
    ...
    // One source of truth: what a provider is told about a tool is what gates it.
    for (const tool of tools) {
      const annotations = tool.definition.annotations as { readOnlyHint?: boolean } | undefined;
      expect(computerToolRequiresApproval(tool.definition.name)).toBe(
  ...
  it("shapes every tool's input the way its targeting and observation need", async () => {
    const { byName } = await setup();
    const properties = (name: string) =>
      Object.keys(
        (byName.get(name)?.definition.inputSchema as { properties?: Record<string, unknown> })
          ?.properties ?? {},
      );
    // The optional screenshot id lives beside x/y on every pointer tool.
    for (const name of [
      "computer_click",
      "computer_double_click",
      "computer_triple_click",
      ...
      expect(properties(name)).toEqual(expect.arrayContaining(["x", "y", "screenshot_id"]));
    }
    // Keyboard tools aim by window id; observed actions can opt out of the
    // capture, and the two that capture nothing do not offer to.
    for (const name of ["computer_type_text", "computer_press_key", "computer_hotkey"]) {
      expect(properties(name)).toEqual(expect.arrayContaining(["window_id", "include_screenshot"]));
    }
    for (const name of [
      "computer_click",
      "computer_move_cursor",
      ...
      expect(properties(name)).toContain("include_screenshot");
    }
    for (const name of ["computer_launch_app", "computer_write_clipboard"]) {
      expect(properties(name)).not.toContain("include_screenshot");
    }
  });

  it("advertises the semantic actions this desktop's family accepts", async () => {
    const linux = await setup();
    expect(schemaEnum(linux.byName, "computer_perform_action", "action")).toEqual([
      "activate",
      "click",
    ]);
    const mac = await setup(new FakeComputerBackend({ agentDialect: "macos" }));
    expect(schemaEnum(mac.byName, "computer_perform_action", "action")).toEqual(
      expect.arrayContaining(["activate", "click", "AXShowMenu", "AXScrollToVisible"]),
    );
  });
  ...
  it("refuses a hotkey chord past the contract's shape before dispatch", async () => {
    ...
    const longKey = await call("computer_hotkey", {
      keys: ["k".repeat(COMPUTER_KEY_NAME_MAX_LENGTH + 1)],
    });
  ...
  it("tells the model why an approval was declined when the gate gives a reason", async () => {
    const backend = new FakeComputerBackend();
    const manager = new ComputerManager({ backend, actionSettleMs: 0 });
    try {
      const tools = makeAgentGatewayComputerTools({
        manager,
        authorizeAction: async () => ({ decision: "decline", reason: "Declined for a reason." }),
      });
      const tool = tools.find((entry) => entry.definition.name === "computer_click")!;
      const result = await Effect.runPromise(
        tool.handler({ x: 1, y: 1 }, makeContext("antigravity")),
      );
      expect(result.isError).toBe(true);
      expect(result.content[0]).toMatchObject({
        text: expect.stringContaining("Declined for a reason."),
      });
      expect(backend.callsFor("click")).toHaveLength(0);
    } finally {
      await manager.dispose();
  ...
  it("sends a triple click as one gesture, and refuses where it cannot be one", async () => {
    ...
    // backend that cannot express the gesture — the fake, like the Linux
    // backends — says so rather than approximating.
    const { backend, call, see } = await setup();
    await see();
    const refused = await call("computer_triple_click", { x: 40, y: 40 });
    expect(refused.isError).toBe(true);
    ...
    const capable = Object.assign(new FakeComputerBackend(), {
      tripleClick: async (point: unknown) => {
        gestures.push(point);
        return { point };
      },
    });
    const able = await setup(capable);
    await able.see();
    await able.call("computer_triple_click", { x: 40, y: 40, include_screenshot: false });
    expect(gestures).toEqual([{ x: 50, y: 50 }]);
    expect(capable.callsFor("click")).toHaveLength(0);
  });
...
describe("screenshot delivery consistency", () => {
  it("refreshes screenshot coordinates when an unchanged window moves", async () => {
    ...
    try {
      ...
      const windows = await backend.listWindows();
      backend.emitWindowsChanged(
        // Preserve the earlier window snapshot so movement can invalidate it.
...
it("keeps computer metadata compact", async () => {
  const { manager, call } = await setup(new FakeComputerBackend({ agentDialect: "macos" }));
  try {
