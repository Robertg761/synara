...
export class HelperClient {
  ...
  start(): void {
    ...
    this.process = child;
    this.exited = false;
    const writer = new JsonRpcStdioWriter(child.stdin);
    ...
    const framer = new JsonRpcStdioFramer(MAX_CONTROL_LINE_BYTES, (error) =>
      this.handleControlLineError(error, registry),
    );
    this.stdinWriter = writer;
    this.requestRegistry = registry;
    registry.processStarted();

    // Every listener below is bound to the process it was installed for and to
    // that process's own framer, writer and registry. Node keeps delivering
    // buffered `data` after a child is replaced or disposed, and the listeners
    // used to read whichever transport `this` happened to hold: a dying helper
    // could answer a request issued to its successor, or reject the
    // successor's in-flight calls with its own exit.
    child.stdout.on("data", (chunk: Buffer) => {
      if (this.process !== child) return;
      this.consumeStdout(chunk, framer, registry);
    });
    ...
    child.on("error", (error) => {
      if (this.process !== child) return;
      this.fail(new DeviceHelperError("helper_spawn_failed", error.message), registry, writer);
    });
    child.on("exit", (code, signal) => {
      ...
      this.fail(new DeviceHelperError("helper_exited", reason), registry, writer);
      this.options.onExit?.(reason);
    });
  }

  async request(method: string, params: Record<string, unknown> = {}): Promise<unknown> {
    // A disposed client must stay disposed. Restarting the helper from here
    // resurrected a process its owner had already torn down, against a device
  ...
  private consumeStdout(
    chunk: Buffer,
    framer: JsonRpcStdioFramer,
    registry: JsonRpcStdioRequestRegistry,
  ): void {
    try {
      for (const line of framer.push(chunk)) {
        const trimmed = line.trim();
        if (trimmed.length > 0) this.handleControlLine(trimmed, registry);
      }
    } catch (error) {
      ...
      registry.rejectAll(new DeviceHelperError("helper_protocol_error", message, { cause: error }));
    }
  }
   ...
   */
  private handleControlLineError(
    error: JsonRpcStdioTransportError,
    registry: JsonRpcStdioRequestRegistry,
  ): void {
  ...
  private handleControlLine(line: string, registry: JsonRpcStdioRequestRegistry): void {
    let message: unknown;
    try {
   ...
   */
  private fail(
    error: DeviceHelperError,
    registry = this.requestRegistry,
    writer = this.stdinWriter,
  ): void {
    registry?.processExited(error);
    writer?.close(error);
  }
}
