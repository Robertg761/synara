...
vi.mock("node:child_process", () => {
  ...
  class FakeEventEmitter {
    ...
    emit(event: string, ...args: readonly unknown[]): boolean {
      ...
      // Dispatch the original listeners even if a callback changes subscriptions.
...
describe("helper stdio control channel", () => {
  ...
  it("refuses to work after dispose instead of resurrecting the helper", async () => {
    const client = makeControlClient((child, request) => {
    ...
    // `request` used to start a fresh helper whenever none was running, so a
    // caller holding a disposed client silently spawned a new process against
    // an attachment its owner had already torn down.
  ...
  it("stops reporting a helper it was asked to dispose", async () => {
    let child:
      | { readonly stdout: { emit: (event: string, ...args: readonly unknown[]) => boolean } }
      | undefined;
    const exits: string[] = [];
    nextResponseHandler = (spawned, request) => {
      child = spawned;
      spawned.stdout.emit(
        "data",
        Buffer.from(`${JSON.stringify({ id: request.id, result: "ok" })}\n`),
    ...
    // The kill that dispose issues comes back as an exit, and a retired helper
    // keeps flushing whatever it had already written. Neither is news: the
    // owner asked for this, and the transport it would be reporting against is
    // already gone.
    expect(exits).toEqual([]);
    expect(() =>
      child?.stdout.emit("data", Buffer.from(`${JSON.stringify({ id: 1, result: "late" })}\n`)),
    ).not.toThrow();
  });
