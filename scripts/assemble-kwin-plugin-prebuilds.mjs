...
/**
 * The toolchain fields the build step records. Optional, because an image
 * whose cmake config could not be read still yields a build that is correct
 * on its own distribution; an empty string means "unknown" and is dropped so
 * the runtime manifest reader sees absence rather than a blank version.
 */
const TOOLCHAIN_FIELDS = ["qtVersion", "kfVersion", "glibcVersion"];

function toolchainOf(metadata, path) {
  const recorded = {};
  for (const field of TOOLCHAIN_FIELDS) {
    const value = metadata[field];
    if (value === undefined || value === "") continue;
    if (!text(value)) throw new Error(`Invalid ${field} in KWin prebuild metadata: ${path}`);
...
function readMetadata(path) {
  ...
  return {
    kwinVersion: metadata.kwinVersion,
    arch: metadata.arch,
    builtOn: metadata.builtOn,
    file: metadata.file,
    ...toolchainOf(metadata, path),
  };
}
