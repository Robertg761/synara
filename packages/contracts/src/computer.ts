...
export const COMPUTER_WS_METHODS = {
  ...
  // User-driven input from the computer dock pane. The agent's own actions
  // never cross this surface — they go through the agent gateway's MCP tools —
  // so the pane's methods are the only mutating ones here: they must work with
  // no agent turn in flight, and a pane only ever sends resolved desktop
  // coordinates, never the semantic (label/role) targets the tools resolve.
...
/**
 * Longest key name any input surface accepts: one printable character or a
 * named key such as `arrowleft`, whether it arrives as a pressed key, one key
 * of a chord, or a pane keystroke. One bound for all three, because the pane
 * and the tools feed the same backend vocabulary and a name one surface accepts
 * and the other refuses is a difference nothing in the product means.
 */
export const COMPUTER_KEY_NAME_MAX_LENGTH = 64;
/**
 * The accessibility tree's shape budget, applied to every tree that travels:
 * the deepest node (which is also the longest `nodePath` a backend may
 * address), the widest node, and the whole tree. A walk is bounded by the
 * backend's own budget first; these are the contract's ceiling on what any
 * backend may hand a payload, so the server clamps to them before encoding.
 */
export const COMPUTER_UI_TREE_MAX_DEPTH = 64;
export const COMPUTER_UI_NODE_MAX_CHILDREN = 2_048;
export const COMPUTER_UI_TREE_MAX_NODES = 32_768;
...
export const ComputerHealth = Schema.Struct({
  ...
  /**
   * The desktop session is locked or inactive, so the backend is connected
   * but refusing capture and input until the human unlocks it. Its own field
   * rather than a health failure, because nothing is broken: a pane shows a
   * lock, not a reconnect, and an agent waits rather than retries. Optional
   * because only backends that can see the session lock report it.
   */
...
export const ComputerWindow = Schema.Struct({
  ...
  occludedBy: Schema.optional(
    Schema.Array(ComputerWindowId).check(Schema.isMaxLength(COMPUTER_OCCLUDERS_MAX_LENGTH)),
  ),
});
...
export const ComputerUiNode: Schema.Schema<ComputerUiNode> = Schema.Struct({
  ...
  nodePath: Schema.optional(
    Schema.Array(NonNegativeInt).check(Schema.isMaxLength(COMPUTER_UI_TREE_MAX_DEPTH)),
  ),
  ...
  children: Schema.Array(Schema.suspend((): Schema.Schema<ComputerUiNode> => ComputerUiNode)).check(
    Schema.isMaxLength(COMPUTER_UI_NODE_MAX_CHILDREN),
  ),
});

/**
 * Whether a whole tree fits the depth and node budgets. Checked once at the
 * root rather than on every node, because a per-node check would re-walk each
 * subtree once per ancestor.
 */
function computerUiTreeWithinBudget(root: ComputerUiNode): boolean {
  let nodes = 0;
  ...
  while (stack.length > 0) {
    ...
    nodes += 1;
    if (nodes > COMPUTER_UI_TREE_MAX_NODES || depth > COMPUTER_UI_TREE_MAX_DEPTH) return false;
    for (const child of node.children) stack.push({ node: child, depth: depth + 1 });
  }
  return true;
}

/** A tree root as it may travel: every node valid, and the whole within budget. */
export const ComputerUiTree: Schema.Schema<ComputerUiNode> = ComputerUiNode.check(
  Schema.makeFilter(computerUiTreeWithinBudget, { identifier: "ComputerUiTreeBudget" }),
);
...
export const ComputerState = Schema.Struct({
  ...
  root: Schema.optional(ComputerUiTree),
...
export const ThreadComputerState = Schema.Struct({
  ...
  /**
   * Whether this backend has anything to install on request, the same fact
   * `ComputerStatusResult` carries. On the thread state so the pane can decide
   * whether to offer "Set up" from the snapshot it is pushed, without pairing
   * it with a status query. Optional for older servers.
   */
...
export const ComputerPressKeyInput = Schema.Struct({
  key: TrimmedNonEmptyString.check(Schema.isMaxLength(COMPUTER_KEY_NAME_MAX_LENGTH)),
});
