...
describe("shared JSON-RPC stdio transport", () => {
  ...
  it("hands back a chunk's good lines alongside its failures instead of throwing", () => {
    // The default handler rethrows, which used to discard every line the chunk
    // had already decoded. `pushRecords` leaves that decision to the caller.
    const framer = new JsonRpcStdioFramer(64);
