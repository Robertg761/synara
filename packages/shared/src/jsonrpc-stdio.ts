...
/** Everything one chunk produced: the lines it completed and the lines it lost. */
export interface JsonRpcStdioFramedChunk {
  readonly frames: ReadonlyArray<string>;
  readonly errors: ReadonlyArray<JsonRpcStdioTransportError>;
}
...
export class JsonRpcStdioFramer {
  ...
  /**
   * Frames a chunk and reports its dropped lines through `onLineError`.
   *
   * Convenience over `pushRecords` for callers whose line-error policy never
   * throws. A handler that does throw costs the caller the lines this chunk had
   * already decoded, which is why anything treating a framing failure as
   * session-ending should frame through `pushRecords` instead and decide after
   * it has drained the good lines.
   */
  push(chunk: Buffer | Uint8Array | string): ReadonlyArray<string> {
    const { frames, errors } = this.pushRecords(chunk);
    for (const error of errors) this.onLineError(error);
    return frames;
  }

  /**
   * Frames a chunk, returning both the lines it completed and the lines it lost.
   *
   * A line that cannot be framed costs exactly that line: its bytes are dropped
   * and the scan continues to the next newline, across chunks if the line is
   * still arriving. Failures are collected rather than thrown because the scan
   * has to reach the end of the chunk either way — bytes left unread here are
   * bytes that desynchronize the next chunk.
   */
  pushRecords(chunk: Buffer | Uint8Array | string): JsonRpcStdioFramedChunk {
    if (this.ended) {
